...
{
	var avplay1=Player.GetBitrates();
    if( this.bRunTimer == 1 )
    {     
        var curBandwidth 	= 1;
		var contentBitrate 	= avplay1.getCurrentBitrate();//this.pluginPlayer.Execute("GetCurrentBitrates");
		alert("contentBitrate==="+contentBitrate);
...
Bitrate.initBitrateLevel = function()
{
	...
	var avplay1=Player.GetBitrates();
    var bitrates = avplay1.getAvailableBitrates();//this.pluginPlayer.Execute("GetAvailableBitrates");
    //document.getElementById("description").innerHTML="bitrates============="+bitrates;
	if( bitrates.length <= 0 )
	{
		return;
	}
	this.print("Bitrate.initBitrateLevel : " + bitrates);   
	document.getElementById("description").innerHTML +=bitrates;
	
    this.bitrateLevelArray =bitrates;//bitrates.split("|");
